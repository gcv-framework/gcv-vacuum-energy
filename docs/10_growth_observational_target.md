# Growth Sector in GCV

> **Current status - 27 July 2026:** Superseded as a derived nonzero GCV prediction. The archived response law and vectors remain conditional benchmarks; no additional growth deformation is presently derived beyond standard GR+`Lambda_eff` growth. Within the archived response parametrization, the current extra-response baseline is `epsilon = 0`.

## Role

Historical growth-sector rank-clock response, retained visible-source activation, and falsifier-interface metadata record.

## Canonical record

- **Current title:** Growth Sector in GCV: Rank-Clock Response, Retained Visible-Source Activation, and Falsifier Interface
- **Concept DOI / module family:** [10.5281/zenodo.18933387](https://doi.org/10.5281/zenodo.18933387)
- **Current canonical version DOI:** [10.5281/zenodo.20128073](https://doi.org/10.5281/zenodo.20128073)
- **Current canonical release date:** 2026-05-12
- **Current version:** v2.0.0

## Priority / first public version

- **First public version DOI:** [10.5281/zenodo.18933387](https://doi.org/10.5281/zenodo.18933387)
- **First public version title:** A growth-sector observational target for GCV on the GR-exact branch: Fixed ruler-free geometry, small coherent perturbation-sector response
- **First public version release date:** 2026-03-12
- **Earliest public timestamp date:** 2026-03-12
- **First timestamp DOI/URL:** —

## OSF provenance

- **OSF backup project:** [OSF backup](https://osf.io/bgms8/)
- **OSF v1 registration:** [OSF registration](https://osf.io/em6cu/)
- **OSF backup date:** 2026-05-12
- **OSF registration date:** 2026-05-12
- **Shared mirror / provenance coverage:** —

## Notes

- **Priority tier:** Medium
- **Journal/submission status:** not submitted
- **Title changed:** Yes
- **Title-change notes:** May 2026 metadata update to the current v2.0.0 Growth Sector title.
- **Other notes:** Preserve the concept DOI separately from the current version DOI.
